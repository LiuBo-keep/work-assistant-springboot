...
public class HrmsCardRecordPullManager {
  ...
  private void pull(CheckInType checkInType) {
    ...
    try {
      ...
      // 只处理当天数据
      if (!record.getClockInTime().toLocalDate().equals(LocalDate.now())) {
        return;
      }
  ...
  private boolean sameMinute(LocalDateTime t1, LocalDateTime t2) {
    return t1.truncatedTo(ChronoUnit.MINUTES)
        .equals(t2.truncatedTo(ChronoUnit.MINUTES));
  }
}
